...
#' @details (4) temp --- temperary dir in which files are not necessary, you can neglect it
#' @return Generate amounts of "tif" files to outpath
#' @export
hp_CHN <- function(startmon,endmon,resol,configfile,outpath,sys,para,newHouse){
  ...
  ###########################################################################################
  ############## calculate the months, and create the month **************** ################
  ###########################################################################################
  ...
  if (para){
    if (sys == "wins"){
      ...
      foreach (i=24:nrow(cityinfo)) %dopar% 
      {
        hp_city(cityinfo$pinyinabb[i],cityinfo$host[i],cityinfo$port[i],
                ...
                startmon,endmon,resol,outpath,sys,newHouse)
      }
      stopCluster(cl)
    }
  }else{
    for (i in 1:nrow(cityinfo))
    {
      ...
      hp_city(district,host,port,user,password,dbname,startmon,endmon,resol,outpath,sys,newHouse)
