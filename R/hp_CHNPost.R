...
#' @export
hp_CHNPost <- function(mon,resol,configfile,outpath,sys,para,newHouse){
  ...
  if (para){
    if (sys == "wins"){
      ...
      foreach (i=24:nrow(cityinfo)) %dopar% 
      {
        hp_cityPost(cityinfo$pinyinabb[i],cityinfo$host[i],cityinfo$port[i],
                ...
                mon,resol,outpath,sys,newHouse)
      }
      stopCluster(cl)
    }
  }else{
    for (i in 1:nrow(cityinfo))
    {
      ...
      hp_cityPost(district,host,port,user,password,dbname,mon,resol,outpath,sys,newHouse)
